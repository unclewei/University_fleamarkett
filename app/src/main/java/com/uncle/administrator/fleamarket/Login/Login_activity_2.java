                ...
                if (ex == null) {//短信验证码已验证成功
                    ...
                    bomb_openhelper.findAccount(nub, new BOMBOpenHelper.FindAccountCallback() {
                        ...
                        public void onSuccess(List<Profile> list) {
                            Profile userAccount = list.get(0);
                        ...
                        public void onFail(int failCode) {//在数据库中找不到账号的，创建新的账号，并且登录跳转
                            if (failCode == 9015) {
                                final Profile profile = new Profile(null,
                                        "鱼塘小虾", "五邑大学", "计算机学院",
                                        null, null, null);
                                bomb_openhelper.addAccount(profile, new BOMBOpenHelper.AddAccountCallback() {
                                    @Override
                                    public void onSuccess(String object) {
                                        profile.setObjectId(object);
                                        SPUtil.getInstance(Login_activity_2.this).saveSP("account", "profile", new Gson().toJson(profile).toString());
                                        SharedPreferences sharedPreferences = getSharedPreferences("account", Context.MODE_WORLD_READABLE);
                                        handler.sendEmptyMessage(0);
